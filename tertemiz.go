...
func Tertemiz(metin string) string {
	tmark := transform.Chain(norm.NFD, runes.Remove(runes.In(unicode.Mn)), norm.NFC)
	...
	r := strings.NewReplacer(" ", "-", "ı", "i", "I", "i")

	temiz := strings.ToLowerSpecial(unicode.TurkishCase, metin)
	temiz, _, _ = transform.String(tmark, temiz)
	temiz = r.Replace(temiz)
	temiz = rx.ReplaceAllString(temiz, "")

	return strings.Trim(temiz, "-")
}
